...
#loading the dataset pulled from GitHub
```{r}
library(dplyr)
rain.df<-read.table("~/Library/Mobile Documents/com~apple~CloudDocs/Brown/2560 R/In Class Week 2:Katie Gately/rnf6080.dat")
View(rnf6080.dat)
```
...
#Checking the dimensions of the data table using the dim function.
```{r}
dim(rain.df)
```
#There is a correct amount of rows and columns

3. What are the names of the columns of rain.df?

#Finding the names of the columns using the names function.
```{r}
names(rain.df)
```
#The columns are names V1-V27. 
...
rain.df[5,7]
```
#The value of row 5, column 7 is 0. 
...
```
#The command changes the names of the first three columns to "year", "month", and "day" and changes the other columns to names 0-23. The last 24 columns represent the hours in the day. 
...
#Created a new column that is the sum of columns 0-23. The histogram shows that some of the values are negative, which is not possible.
```{r}
daily = rain.df %>%
  select(-1, -2, -3)%>%
  rowSums()
  rain<-cbind(rain.df, daily)
hist(rain$daily,main="Frequency of Rain per Hour")
```
...
#Some of the values are negative so we had to mutate the data to replace any negative values with NA. This is more reasonable because it is not possible to have negative rainfall, which was most likely a measurement error. 
```{r}
rain.df.fixed<- rain %>% 
...
#Filtered the dataset to show only values for flights on August 31st. 
```{r}
flights %>%
...
#Arranged the dataset by arrival delay in descending order. 
```{r}
flights %>%
